...
A third shape exists - the **reference-mode** tool. It has `"mode": "reference"`, NO `invoke`
and no script: its `instructions` ARE the tool. Running it hands the instructions back to the
caller (still through the PreToolUse gate - a deny withholds them) instead of executing anything
server-side. Use it for run-it-yourself recipes: a CLI invocation the agent should run in its own
environment, a checklist, a procedure.

```json
{
  "id": "deploy-runbook",
  "name": "Deploy Runbook",
  "summary": "Step-by-step deploy procedure for this project.",
  "mode": "reference",
  "instructions": "1. npm test - suite must be green. 2. npm run build. 3. ..."
}
```
