...
| `command` / `args` / `env` | string / string[] / object | The process the aggregator spawns for a `stdio` upstream. Paths must point **inside the project's vendor directory** (clone vendored servers in; never reference servers configured elsewhere on the host). |
...
1. Add an entry to `upstreams[]` with a unique `id`, the `stdio` `command`/`args`, and
   `enabled: true`. Vendor the server **inside the project's vendor directory** if it isn't already.
