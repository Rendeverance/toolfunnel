...
```
dist/my-mcp/
  package.json      # name, bin, files, dependencies: { "toolfunnel": "^<current version>" }
...
- security and bug fixes reach every wrapped MCP through a normal `npm update` (the pack pins the caret range of the version that built it, so patch releases flow automatically) - no stale forks
  carrying old bugs;
